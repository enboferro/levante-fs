...
with t1:
    ...
    st.markdown(f"""
        ...
        <div class="fouls-bar">FALTAS LUD: {s.fl} | RIVAL: {s.fr} | {s.periodo}</div>
    """, unsafe_allow_html=True)
    ...
    with c_top[3]:
        with st.popover("⚽ RIV", use_container_width=True):
            ...
            if st.button("GOL RIVAL"): s.mr+=1; capturar_tactico("GOL RIV", f"#{d}"); s.eventos.append({'Tiempo':min_act,'Evento':f'⚽ GOL RIVAL (#{d})'}); st.rerun()
    with c_top[4]: 
    ...
    with f_m:
        ...
        with c2:
            with st.popover("RIV 🟨🟥"):
                ...
                if st.button("🟨 RIV"): s.ar+=1; s.eventos.append({'Tiempo':min_act,'Evento':f'🟨 RIV (#{dy})'}); st.rerun()
                if st.button("🟥 RIV"): s.rr+=1; s.eventos.append({'Tiempo':min_act,'Evento':f'🟥 RIV (#{dy})'}); st.rerun()
        with c3:
            st.write("🧤 Mano")
            c_m = st.columns(2)
    ...
    with f_r:
        st.button("⏱️ TM RIVAL", key="tmr", on_click=lambda: (toggle_timer(), setattr(s, 'tm', True), setattr(s, 'tm_i', time.time())))
        c_f2 = st.columns(2)
        c_f2[0].button("F+ RIV", on_click=lambda: setattr(s, 'fr', s.fr+1))
        c_f2[1].button("F- RIV", on_click=lambda: setattr(s, 'fr', max(0, s.fr-1)))
    st.markdown("</div>", unsafe_allow_html=True)
...
with t4:
    ...
    st.download_button(label="📥 DESCARGAR INFORME EXCEL (.xlsx)", data=buf.getvalue(), file_name=f"LUD_Partido.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
