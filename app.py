...
st.markdown("""<style>
    .stApp{background:#F0F2F6}.h-box{background:linear-gradient(90deg,#003D7A,#7A0019);padding:10px;border-radius:10px;color:white;text-align:center}
    .rel-box{background:#1E1E1E;border:3px solid #FFD700;border-radius:20px;padding:10px}.rel-txt{color:white;font-size:4rem!important;font-weight:bold;text-align:center;margin:0}
    .m-card{padding:15px;border-radius:15px;color:white;text-align:center;min-height:180px}.stButton>button{height:3rem;font-weight:bold}
    .j-card{background:white;padding:8px;border-radius:10px;border-left:8px solid #D3D3D3;margin-bottom:5px}
    .pista-contador {text-align:center; padding:10px; background:white; border-radius:10px; border:2px solid #003D7A; margin-top:10px}
...
with st.container():
    ...
    with ci:
        r1, r2 = st.columns(2)
        rvn = r1.text_input("RIVAL", "RIVAL", label_visibility="collapsed").upper()
        fp = r2.date_input("FECHA", datetime.now(), label_visibility="collapsed")
...
with c1:
    ...
    la, lb = st.columns(2); la.button("⚽ GOL", on_click=lambda: setattr(ss, 'ml', ss.ml+1), key="gl")
    lb.button("⚠️ FALTA", on_click=lambda: setattr(ss, 'fl', ss.fl+1), key="fl")
...
with c3:
    ...
    ra, rb = st.columns(2); ra.button("⚽ GOL ", on_click=lambda: setattr(ss, 'mr', ss.mr+1), key="gr")
    rb.button("⚠️ FALTA ", on_click=lambda: setattr(ss, 'fr', ss.fr+1), key="fr")

# POSESIÓN Y CONTADOR PISTA
st.divider()
...
if p3.button(f"🔴 {rvn}", key="pr", use_container_width=True, type="primary" if ss.pos=="RIV" else "secondary"): ss.pos="RIV"; st.rerun()

jugadores_en_pista = sum(1 for j in ss.js if j["p"])
color_pista = "red" if jugadores_en_pista > 5 else "#003D7A"
st.markdown(f"<div class='pista-contador'><h3 style='color:{color_pista}; margin:0;'>👥 JUGADORES EN PISTA: {jugadores_en_pista} / 5</h3></div>", unsafe_allow_html=True)
    ...
    for idx, col in enumerate(cols):
        ...
        if ji < 14:
            ...
            with col:
                ...
                t.write(f"⏱️{mj:02d}:{sj:02d}")
                if e.button("🔴" if j["p"] else "🟢", key=f"bt{ji}"):
                    if not j["p"] and jugadores_en_pista < 5:
                        j["p"], j["e"] = True, j["e"]+1
                        if ss.ac: j["ini"] = now
                    elif j["p"]:
                        if ss.ac and j["ini"]:
                    ...
                    elif not j["p"] and jugadores_en_pista >= 5:
                        st.error("¡Ya hay 5 jugadores!")
                    st.rerun()
                s1, s2, s3, s4 = st.columns(4)
                if s1.button(f"🎯{j['t']}", key=f"
