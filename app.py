...
st.markdown("""
    ...
    .block-container { padding: 0.5rem !important; max-width: 1400px; margin: 0 auto; }
    ...
    /* ESTILO CHIPS MARCADOR */
    .mini-chip-container { display: flex; justify-content: center; gap: 4px; margin-top: 4px; }
    .mini-chip { padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; font-weight: 800; background: #333; color: #fff; border: 1px solid rgba(255,255,255,0.2); }
    .bonus-alert { background-color: #ff0000 !important; animation: blink 0.8s infinite; }
    @keyframes blink { 0% {opacity: 1;} 50% {opacity: 0.3;} 100% {opacity: 1;} }

    /* ALINEACIÓN DE COLUMNAS */
    [data-testid="column"] { align-items: center !important; justify-content: center !important; text-align: center !important; }
    </style>
...
# --- MARCADOR PRINCIPAL (TODO ORGANIZADO EN UNA FILA) ---
c_main = st.columns([1.5, 3, 1.5])
...
# LÍNEA DE TIEMPO (BAJO EL MARCADOR)
if s.gi:
    tl_html = "<div style='text-align:center; margin-top:5px;'>"
    for g in s.gi:
        bg = "#003D7A" if g["team"]=="LUD" else "#fff"
        col = "#fff" if g["team"]=="LUD" else "#333"
        tl_html += f"<span style='background:{bg}; color:{col}; padding:2px 8px; border-radius:4px; font-size:0.8rem; margin:2px; border:1px solid #ccc;'>{g['m']}' ⚽</span>"
    st.markdown(tl_html + "</div>", unsafe_allow_html=True)

# CONFIGURACIÓN RÁPIDA
with st.expander("⚙️ CONFIGURACIÓN PARTIDO"):
    c_cfg = st.columns(3)
    s.rv = c_cfg[0].text_input("Nombre Rival", s.rv).upper()
    s.fe = c_cfg[1].text_input("Fecha", s.fe)
    if c_cfg[2].button("🗑️ REINICIAR TODO"): st.session_state.clear(); st.rerun()

# --- GRID JUGADORES (ROTACIONES Y Σ TOTAL) ---
st.markdown("<div style='margin:10px 0;'></div>", unsafe_allow_html=True)
cols_j = st.columns(5)
for idx, j in enumerate(s.js):
    with cols_j[idx%5]:
        status_class = "pista-activa" if j['p'] else "banquillo-espera"
        color_sub = "rgba(255,255,255,0.85)"
        with st.container():
            ...
            st.markdown(f"<div style='font-size:0.95rem; font-weight:900;'>{j['n']} <span style='float:right;'>R:{j['r']}</span></div>", unsafe_allow_html=True)
            st.markdown(f"<div style='font-size:1.8rem; font-weight:900; margin:2px 0;'>{mj:02d}:{vj:02d}</div>", unsafe_allow_html=True)
