...
st.set_page_config(page_title="LUD Match Control v22.1", layout="wide")
...
def capturar_cuarteto(evento_tipo, detalle):
    # Captura los 4 que no son porteros
    ...
    s.analisis_tactico.append({
        ...
        "P1": cuarteto[0], "P2": cuarteto[1], "P3": cuarteto[2], "P4": cuarteto[3]
    })
...
with tab1:
    ...
    # Botones Control Tiempo
    c_time = st.columns(3)
    for col_key in ["tm_l", "tm_m", "tm_r"]:
        if c_time[["tm_l", "tm_m", "tm_r"].index(col_key)].button("▶ START / STOP ⏸", key=col_key):
            toggle_timer(); st.rerun()

    # Goles y Configuración
    ...
    with c_goles[0]:
        with st.popover("⚽ GOL LUD", use_container_width=True):
            ...
            if st.button("GOOOL!"):
                s.ml += 1; capturar_cuarteto("GOL LUD", p_gol); s.eventos.append({'min':min_act,'info':f'⚽{p_gol}'}); st.rerun()
    with c_goles[1]:
        with st.popover("⚽ GOL RIVAL", use_container_width=True):
            d_gol = st.number_input("Dorsal", 1, 99, key="ni_gol_riv")
            if st.button("GOL RIVAL"):
                s.mr += 1; capturar_cuarteto("GOL RIVAL", f"#{d_gol}"); s.eventos.append({'min':min_act,'info':f'⚽#{d_gol} RIV'}); st.rerun()
    ...
    with f_l: # LUD
        ...
        with c_tml:
            if st.button("⏱️ TM LUD", key="bt_tml", use_container_width=True):
                toggle_timer(); s.tm, s.tm_i = True, time.time(); capturar_cuarteto("TM LUD", "Iniciado"); st.rerun()
        with c_fl:
    ...
    with f_m: # Disciplina y Portería
        ...
        # Stats portería
        c_stats = st.columns(2)
        c_stats[0].button(f"🧤 {s.pm}", key="bt_pm", use_container_width=True, on_click=lambda: setattr(s, 'pm', s.pm+1))
        c_stats[1].button(f"👟 {s.pp}", key="bt_pp", use_container_width=True, on_click=lambda: setattr(s, 'pp', s.pp+1))
    ...
    with f_r: # RIVAL
        ...
        with c_tmr:
            if st.button("⏱️ TM RIVAL", key="bt_tmr", use_container_width=True):
                toggle_timer(); s.tm, s.tm_i = True, time.time(); capturar_cuarteto("TM RIV", "Iniciado"); st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
...
with tab2:
    ...
    if s.analisis_tactico:
        ...
        st.table(df_tactica)
        # Botón para descargar CSV rápido
        csv = df_tactica.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Descargar Datos Tácticos", csv, "analisis_lud.csv", "text/csv")
    else:
        st.info("Registra goles, tarjetas o tiempos muertos para ver el análisis de cuartetos.")
