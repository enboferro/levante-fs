...
st.set_page_config(page_title="LUD v5.3", layout="wide")
st.markdown("<style>.block-container{padding-top:0.5rem;} [data-testid='stVerticalBlock']>div{gap:0.1rem;} div.stButton>button{border-radius:5px;height:2em;font-size:0.8rem!important;} .title{text-align:center;color:#003D7A;font-size:1.6rem;font-weight:bold;border-bottom:2px solid #ed1c24;} .label-x{font-size:0.7rem;font-weight:bold;text-align:center;color:#666;margin-top:2px;}</style>", unsafe_allow_html=True)
...
s = st.session_state
if not s.ex: st_autorefresh(1000, key="f5_v53")
ah = time.time()
...
with c1:
    ...
    # FILA COMPARTIDA: PORTERO Y DUELOS
    p_col, d_col = st.columns(2)
    with p_col:
        st.markdown("<div class='label-x'>🧤 PORTERO</div>",1)
        px1,px2 = st.columns(2)
        if px1.button(f"🧤{s.pm}", key="pm"): s.pm+=1; st.rerun()
        if px2.button(f"👟{s.pp}", key="pp"): s.pp+=1; st.rerun()
    with d_col:
        tot_d = s.dok + s.dko
        perc = (s.dok/tot_d*100) if tot_d>0 else 0
        st.markdown(f"<div class='label-x'>⚔️ DUELOS ({perc:.0f}%)</div>",1)
        dx1,dx2 = st.columns(2)
        if dx1.button(f"✅{s.dok}", key="dok"): s.dok+=1; st.rerun()
        if dx2.button(f"❌{s.dko}", key="dko"): s.dko+=1; st.rerun()
with c2:
    mr, sr = divmod(int(rem), 60)
    st.markdown(f"<h1 style='text-align:center;font-size:3.2rem;color:red;margin:0;'>{mr:02d}:{sr:02d}</h1>",1)
    if st.button("▶ START/STOP", use_container_width=True, key="tm", type="primary"):
        if not s.on:
            s.ic, s.on = ah, True
            for j in s.js: 
                if j["p"]: j["i"]=ah
        ...
        st.rerun()
    for g in s.gi[-2:]: st.markdown(f"<p style='font-size:0.75rem;margin:0;text-align:center;'><b>{g['m']}</b> {g['j']}</p>", 1)
with c3:
    ...
    if tr1.button(f"🟨 {s.ar}", key="tr"): s.ar+=1; st.rerun()
    if tr2.button(f"🟥 {s.rr}", key="rr"): s.rr+=1; st.rerun()
st.divider()
cols = st.columns(5)
...
with t2:
    ...
    if st.button("📊 EXCEL"):
        ...
        dt.append({"Jugador":"Duelos %","Min":f"{perc:.2f}%"})
        out = io.BytesIO()
