...
# 1. Inicialización Robusta (Asegura que todas las variables existan)
...
# 3. Lógica del Reloj (Corregida para evitar errores)
tg = st.session_state.rt1 if st.session_state.pt=="T1" else st.session_state.rt2
if st.session_state.ac and st.session_state.ig is not None:
    tg += time.time() - st.session_state.ig
...
with cx:
    if not st.session_state.ac:
        ...
        if st.button("⏸ STOP RELOJ", type="secondary", use_container_width=True):
            ...
            now = time.time()
            if st.session_state.pt=="T1": 
                st.session_state.rt1 += now - st.session_state.ig
            else: 
                st.session_state.rt2 += now - st.session_state.ig
