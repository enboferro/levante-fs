...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LUD Control v36.8", layout="wide")
...
st.markdown("""
    ...
    .porteria-section { background: #ffffff; padding: 10px; border-radius: 10px; border: 2px solid #4B2E2A; margin-top: 20px; }
    .stats-box { background: #e9ecef; padding: 15px; border-radius: 8px; text-align: center; border: 1px solid #ced4da; }
    </style>
    """, unsafe_allow_html=True)

st_autorefresh(1000, key="refresh_v36_8")
ah = time.time()
...
with tabs[0]:
    ...
    if p1.button("🧤 MANO ✅", use_container_width=True): s.pm_ok += 1
    if p2.button("🧤 MANO ❌", use_container_width=True): s.pm_err += 1
    if p3.button("👟 PIE ✅", use_container_width=True): s.pp_ok += 1
...
with tabs[2]:
    ...
    # Tabla Jugadores
    res_df = pd.DataFrame([{"Jugador": j['n'], "Goles": j['g'], "Tiempo": f"{int(j['tot']//60):02d}:{int(j['tot']%60):02d}", "Rot": j['r']} for j in jugadores_activos])
    st.table(res_df)
    
    st.divider()
    
    # Sección Porteros abajo
    ...
    with col_pm:
        st.markdown(f"""
            <div class="stats-box">
                <h4>Saques de Mano</h4>
                <h2 style="color: #28a745;">{p_calc(s.pm_ok, s.pm_err)} Éxito</h2>
                <p>✅ {s.pm_ok} Aciertos | ❌ {s.pm_err} Errores</p>
                <p style="font-size: 0.8rem; color: #666;">Total: {s.pm_ok + s.pm_err}</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    with col_pp:
        st.markdown(f"""
            <div class="stats-box">
                <h4>Saques de Pie</h4>
                <h2 style="color: #007bff;">{p_calc(s.pp_ok, s.pp_err)} Éxito</h2>
                <p>✅ {s.pp_ok} Aciertos | ❌ {s.pp_err} Errores</p>
                <p style="font-size: 0.8rem; color: #666;">Total: {s.pp_ok + s.pp_err}</p>
            </div>
        """, unsafe_allow_html=True)
...
with tabs[4]:
    ...
    try:
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            pd.DataFrame([{
                "Rival": s.rv, "Ciudad": s.ciudad, "Fecha": s.fecha, "Goles LUD": s.ml, "Goles Rival": s.mr,
                "Faltas LUD": s.fl, "Faltas Riv": s.fr, "Saques M OK": s.pm_ok, "Saques P OK": s.pp_ok
...
with tabs[5]:
    ...
    s.rv, s.ciudad, s.fecha = c_c1.text_input("Rival", s.rv).upper(), c_c2.text_input("Ciudad", s.ciudad).upper(), c_c3.text_input("Fecha", s.fecha)
    st.divider()
    new_names = []
