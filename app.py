...
if not s.ex:
    st_autorefresh(1000, key="f5_refresh_v491")
...
if d3.button("🗑️ RESET TOTAL", key="k_reset_main", use_container_width=True):
...
with c1: # LUD
    ...
    col_g, col_f1, col_f2 = st.columns([2,1,1])
    with col_g:
        with st.popover("⚽ GOL", use_container_width=True):
            for j in s.js:
                if st.button(j["n"], key=f"gol_lud_{j['n']}"):
                    ...
                    st.rerun()
    if col_f1.button("F+", key="k_flp", use_container_width=True):
        s.fl += 1
        st.rerun()
    if col_f2.button("F-", key="k_flm", use_container_width=True):
        s.fl = max(0, s.fl - 1)
        st.rerun()
...
with c2: # RELOJ
    ...
    st.markdown(f"<h1 style='text-align:center;font-size:3.8rem;color:red;margin:0;line-height:1;'>{m_rem:02d}:{s_rem:02d}</h1>",1)
    if st.button("▶ START / ⏸ STOP", use_container_width=1, key="k_main_timer", type="primary"):
        if not s.on:
            s.ic, s.on = ah, True
            for j in s.js: 
                if j["p"]: j["i"] = ah
...
with c3: # RIVAL
    ...
    if cg_r.button("⚽ GOL", key="k_gr", use_container_width=True):
        ...
        st.rerun()
    if cf1_r.button("F+", key="k_frp", use_container_width=True):
        s.fr += 1
        st.rerun()
    if cf2_r.button("F-", key="k_frm", use_container_width=True):
        s.fr = max(0, s.fr - 1)
        st.rerun()
...
for idx, j in enumerate(s.js):
    with cols[idx%5]:
        with st.container(border=True):
            ...
            if st.button("CAMBIO", key=f"btn_c_{idx}", use_container_width=1):
...
with t_tot:
    st.write(f"**Rival:** {s.rival_name} | **Fecha:** {s.fecha_partido}")
    mc = st.columns(5)
    for idx, j in enumerate(s.js):
        t_total_live = j["t_total"] + (ah - j["i"] if s.on and j["p"] and j["i"] else 0)
        mt, vt = divmod(int(t_total_live), 60)
        mc[idx%5].write(f"**{j['n']}**: {mt:02d}:{vt:02d} (G:{j['g']})")

with t_ex:
    if s.pa=="1T":
        if st.button("🏁 FINALIZAR 1T", key="btn_fin_1t", use_container_width=True):
            if s.on:
                s.ta += ah - s.ic
                for j in s.js:
                    if j["p"] and j["i"]: 
    ...
    if st.button("📊 GENERAR EXCEL", key="btn_gen_excel"):
        ...
        with pd.ExcelWriter(out, engine='xlsxwriter') as w: 
            df_ex.to_excel(w, index=False, sheet_name='Stats')
        st.download_button("📥 DESCARGAR", out.getvalue(), f"LUD_vs_{s.rival_name}.xlsx", key="btn_download_excel")
