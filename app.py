...
st.set_page_config(page_title="LUD Match Control v9.9", layout="wide")

# --- CSS MEJORADO Y CENTRADO ---
st.markdown("""
    <style>
    ...
    .footer-control { background-color: #ffffff; padding: 15px; border-radius: 20px; box-shadow: 0 -5px 20px rgba(0,0,0,0.1); margin-top: 20px; }
...
if 'js' not in st.session_state:
    ...
    st.session_state.eventos = [] # Lista única de goles y tarjetas
...
s = st.session_state
st_autorefresh(1000, key="f5_v9.9")
...
# --- CABECERA Y CRONO ---
st.markdown("<h3 style='text-align:center; color:#003D7A; margin:0;'>LUD MATCH CONTROL v9.9</h3>", unsafe_allow_html=True)
...
# --- LÍNEA DE SUCESOS (GOLES Y TARJETAS) ---
...
if s.eventos:
    for ev in s.eventos:
        ...
        st.markdown(f"<span class='event-badge' style='background:{bg}; color:{tx};'>{ev['min']}' {icon} {ev['info']}</span>", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)

# --- MARCADORES CON MODALES ---
c_score = st.columns([2, 2, 2, 1])
with c_score[0]:
    st.metric("LUD", s.ml)
    with st.popover("⚽ GOL LUD", use_container_width=True):
        p_gol = st.selectbox("Autor", [j['n'] for j in s.js], key="p_gol_l")
        if st.button("Confirmar Gol"):
            s.ml += 1; s.eventos.append({'min':min_act, 'tipo':'G', 'equipo':'LUD', 'info':p_gol}); st.rerun()
with c_score[1]:
    st.metric(s.rv[:8], s.mr)
    with st.popover(f"⚽ GOL {s.rv[:5]}", use_container_width=True):
        d_gol = st.number_input("Dorsal", 1, 99, key="d_gol_r")
        if st.button("Confirmar Gol Rival"):
            s.mr += 1; s.eventos.append({'min':min_act, 'tipo':'G', 'equipo':'RIV', 'info':f"#{d_gol}"}); st.rerun()
with c_score[2]:
...
for idx, j in enumerate(s.js):
    with cols_j[idx%5]:
        ...
        with st.container():
            ...
            st.markdown(f"<div style='font-size:1.7rem; font-weight:900;'>{mj:02d}:{vj:02d}</div>", 1)
            st.markdown(f"<div style='font-size:0.8rem; opacity:0.8;'>Σ {mt:02d}:{vt:02d}</div>", 1)
...
# --- FOOTER CON MODALES DE TARJETAS ---
st.markdown("<div class='footer-control'>", unsafe_allow_html=True)
f_cols = st.columns([3, 4, 3])
with f_cols[0]:
    st.markdown(f"<b>LUD</b> <span style='color:red;'>F: {s.fl}</span>", 1)
...
with f_cols[1]:
    c_t = st.columns(2)
    with c_t[0]: # TARJETAS LUD
        with st.popover("🟨 LUD", use_container_width=True):
            p_a = st.selectbox("Jugador", [j['n'] for j in s.js], key="pal")
            if st.button("Confirmar Amarilla"): 
                s.al+=1; s.eventos.append({'min':min_act, 'tipo':'A', 'equipo':'LUD', 'info':p_a}); st.rerun()
        with st.popover("🟥 LUD", use_container_width=True):
            p_r = st.selectbox("Jugador", [j['n'] for j in s.js], key="prl")
            if st.button("Confirmar Roja"): 
                s.rl+=1; s.eventos.append({'min':min_act, 'tipo':'R', 'equipo':'LUD', 'info':p_r}); st.rerun()
    with c_t[1]: # TARJETAS RIVAL
        with st.popover("🟨 RIVAL", use_container_width=True):
            d_a = st.number_input("Dorsal", 1, 99, key="dar")
            if st.button("Confirmar Amarilla Rival"): 
                s.ar+=1; s.eventos.append({'min':min_act, 'tipo':'A', 'equipo':'RIV', 'info':f"#{d_a}"}); st.rerun()
        with st.popover("🟥 RIVAL", use_container_width=True):
            d_r = st.number_input("Dorsal", 1, 99, key="drr")
            if st.button("Confirmar Roja Rival"): 
                s.rr+=1; s.eventos.append({'min':min_act, 'tipo':'R', 'equipo':'RIV', 'info':f"#{d_r}"}); st.rerun()
with f_cols[2]:
    st.markdown(f"<b>{s.rv[:5]}</b> <span style='color:red;'>F: {s.fr}</span>", 1)
