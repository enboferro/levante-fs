...
st.markdown("""
    ...
    /* Estilo global para títulos, pero lo forzaremos a blanco en los marcadores */
    h1, h2, h3 { color: #003D7A; } 
    </style>
...
# 4. MARCADOR Y RELOJ (CORREGIDO COLOR BLANCO)
with st.container():
    m_col1, m_col2, m_col3 = st.columns([2, 2, 2])
    
    with m_col1:
        # Texto en blanco para LUD
        st.markdown(f"<div style='text-align:center; background:#003D7A; padding:10px; border-radius:10px;'><h2 style='color:white !important; margin:0;'>LUD: {ss.ml}</h2></div>", unsafe_allow_html=True)
        c1, c2 = st.columns(2)
    ...
    with m_col3:
        # Texto en blanco para RIVAL
        st.markdown(f"<div style='text-align:center; background:#7A0019; padding:10px; border-radius:10px;'><h2 style='color:white !important; margin:0;'>RIVAL: {ss.mr}</h2></div>", unsafe_allow_html=True)
        c3, c4 = st.columns(2)
...
# 5. PISTA Y JUGADORES (Se mantiene igual)
st.divider()
