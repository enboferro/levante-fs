...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="LUD Control v37.0", layout="wide")
...
st.markdown("""
    ...
    .faltas-container { 
        ...
        gap: 10px; padding: 2px; border-radius: 0 0 8px 8px; margin-bottom: 5px;
    }
...
st_autorefresh(1000, key="refresh_v37")
ah = time.time()
...
with tabs[0]:
    ...
    if cf2.button("➕", key="fl_p"): s.fl += 1; s.eventos.append({'Tiempo': min_act, 'Evento': 'FALTA LUD', 'Cuarteto': get_cuarteto()}); st.rerun()
    ...
    if cf5.button("➕", key="fr_p"): s.fr += 1; s.eventos.append({'Tiempo': min_act, 'Evento': f'FALTA {s.rv}', 'Cuarteto': get_cuarteto()}); st.rerun()
    ...
    for idx, j in enumerate(jugadores_activos):
        with cols[idx % 3]:
            ...
            if b3.button("🟨", key=f"ty_{idx}", use_container_width=True):
                s.eventos.append({'Tiempo': min_act, 'Evento': f'🟨 Amarilla: {j["n"]}', 'Cuarteto': get_cuarteto()})
            if b4.button("🟥", key=f"tr_{idx}", use_container_width=True):
                s.eventos.append({'Tiempo': min_act, 'Evento': f'🟥 Roja: {j["n"]}', 'Cuarteto': get_cuarteto()})
