...
with ci:
    r1, r2 = st.columns(2)
    rvn = r1.text_input("RIV", "RIVAL", label_visibility="collapsed").upper()
    fp = r2.date_input("FEC", datetime.now(), label_visibility="collapsed")
...
with c1:
    bg1 = "#FF0000" if ss.fl >= 5 else "#003D7A"
    st.markdown(f"<div class='m-card' style='background:{bg1}'><h3>LUD</h3><h1>{ss.ml}</h1><p>F: {ss.fl}</p></div>", unsafe_allow_html=True)
    la, lb = st.columns(2)
    if la.button("⚽ GOL", key="gl1"): ss.ml += 1; st.rerun()
    if lb.button("⚠️ FALTA", key="fl1"): ss.fl += 1; st.rerun()
