...
st.set_page_config(page_title="LUD v5.2", layout="wide")
...
s = st.session_state
if not s.ex: st_autorefresh(1000, key="f5_v52")
ah = time.time()
...
s.rv = d1.text_input("RIVAL", s.rv, key="in_rv").upper()
s.fe = d2.text_input("FECHA", s.fe, key="in_fe")
if d3.button("🗑️ RESET", key="btn_rst_all"): st.session_state.clear(); st.rerun()
st.divider()
    ...
    with cg:
        with st.popover("⚽ GOL", use_container_width=True):
            for i,j in enumerate(s.js):
                if st.button(j["n"], key=f"gol_pop_{i}"):
                    s.ml+=1; j["g"]+=1; s.gi.append({"j":j["n"],"m":f"{mp:02d}:{sp:02d}"}); st.rerun()
    if cf1.button("F+", key="f_l_p"): s.fl+=1; st.rerun()
    if cf2.button("F-", key="f_l_m"): s.fl=max(0,s.fl-1); st.rerun()
    t1,t2 = st.columns(2)
    if t1.button(f"🟨 {s.al}", key="tar_l"): s.al+=1; st.rerun()
    if t2.button(f"🟥 {s.rl}", key="roj_l"): s.rl+=1; st.rerun()
    st.write("🧤 PORTERO")
    p1,p2 = st.columns(2)
    if p1.button(f"🧤 {s.pm}", key="port_m"): s.pm+=1; st.rerun()
    if p2.button(f"👟 {s.pp}", key="port_p"): s.pp+=1; st.rerun()
    st.write("⚔️ DUELOS")
    dok,dko = st.columns(2)
    if dok.button(f"✅ {s.dok}", key="duel_ok"): s.dok+=1; st.rerun()
    if dko.button(f"❌ {s.dko}", key="duel_ko"): s.dko+=1; st.rerun()
with c2:
    mr, sr = divmod(int(rem), 60)
    st.markdown(f"<h1 style='text-align:center;font-size:3.5rem;color:red;margin:0;'>{mr:02d}:{sr:02d}</h1>",1)
    if st.button("▶ START / ⏸ STOP", use_container_width=1, key="main_tmr", type="primary"):
        if not s.on:
            s.ic, s.on = ah, True
            for j in s.js: 
                if j["p"]: j["i"]=ah
...
with c3:
    ...
    if cg_r.button("⚽ GOL", key="gol_riv"): s.mr+=1; s.gi.append({"j":s.rv,"m":f"{mp:02d}:{sp:02d}"}); st.rerun()
    if cf1_r.button("F+", key="f_r_p"): s.fr+=1; st.rerun()
    if cf2_r.button("F-", key="f_r_m"): s.fr=max(0,s.fr-1); st.rerun()
    tr1,tr2 = st.columns(2)
    if tr1.button(f"🟨 {s.ar}", key="tar_r"): s.ar+=1; st.rerun()
    if tr2.button(f"🟥 {s.rr}", key="roj_r"): s.rr+=1; st.rerun()
st.divider()
cols = st.columns(5)
for idx, j in enumerate(s.js):
    with cols[idx%5]:
        with st.container(border=True):
            ...
            if st.button("CAMBIO", key=f"c_btn_{idx}", use_container_width=1):
...
with t2:
    if s.pa=="1T" and st.button("🏁 FIN 1T", key="f_1t_btn", use_container_width=1):
        ...
        st.rerun()
    if st.button("📊 EXCEL", key="ex_gen_btn"):
        s.ex = True
        ...
        st.download_button("📥 DESCARGAR", out.getvalue(), f"LUD_{s.rv}.xlsx", key="dl_btn_fin")
